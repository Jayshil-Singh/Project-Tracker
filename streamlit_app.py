...
if current_user:
    ...
    with st.sidebar:
        ...
        # Initialize profile picture in session state
        ...
        # Professional sidebar menu using custom tab-style buttons
        menu_options = [
            "🏠 Dashboard",
            "📁 Project Tracker", 
            "🗓️ Meeting & MoM Log",
        ...
        # Initialize active menu in session state
        ...
        for option in menu_options:
            ...
            # Use container to create button-like appearance
            with st.container():
                if st.button(
                    option,
                    key=f"nav_{option}",
                ...
                # Apply custom styling via markdown
                if is_active:
                    st.markdown(f"""
                    <style>
                    [data-testid="stButton"] button[kind="secondary"]:has(div:contains("{option}")) {{
                        background: linear-gradient(90deg, #1f77b4 0%, #764ba2 100%) !important;
                        color: white !important;
