...
BACKUPS_DIR = os.path.join(PROJECTS_ROOT, "Backups")
TEMPLATES_DIR = os.path.join(PROJECTS_ROOT, "Templates")
...
db = ProjectOpsDatabase(os.path.join(PROJECTS_ROOT, "ProjectOps-Assistant", "projectops.db"))
    ...
    with tab1:
        with st.expander("Add New Project", expanded=True):
            with st.form("project_form"):
                ...
                uploaded_file = st.file_uploader("Upload Contract/Docs", type=["pdf", "docx", "jpg", "png"])
                ...
                if submitted:
                    ...
                    if uploaded_file is not None:
                        client_dir = os.path.join(CLIENT_PROJECTS_DIR, client_name.replace(" ", "_"))
                        os.makedirs(client_dir, exist_ok=True)
                        file_path = os.path.join(client_dir, uploaded_file.name)
